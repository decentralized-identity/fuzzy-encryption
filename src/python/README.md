...
3. `corpus_size`: This is the size of the set of allowed words. This means that both the origial words and recovery words must be represented by integers in the range 0 .. (`corpus_size` - 1). *This is specified by the user.*
4. `prime`: This a prime number (p) such that `setsize` < p < 2 * `setsize`. *The user does not set this number*, instead it is derived from the `corpus_size` which is specfied by the user.
...
2. `original_words`: A list of unique of integers in the range 0 .. (`corpus_size` - 1) representing the secret words needed to recover the keys.
...
2. `recovery_words`: A guess of the original words. These are unique integers in the range 0 .. `corpus_size` - 1.
3. `key_count`: The number of keys to be generated.

####Another python code we'll discuss is [jverify.py](/Fuzzy-KeyRecovery/dev/kirko/pseudocode/python/jverify.py) :

> This python code is used to verify the schema of the output of `GenerateSecret()`: `state` (a `FuzzyState` object), using its user-saved JSON represenation that can be easily generated by `__repr__()`. It performs the latter by comparing the schema of the saved JSON to the standard `FuzzyState` JSON schema found in [public-schema.json](Fuzzy-KeyRecovery/dev/kirko/pseudocode/python/JSON_Test/public-schema.json).
<br/>
---
### Tests and Demos 
